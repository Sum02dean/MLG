...
IGV only accept gct format expression data(https: // software.broadinstitute.org/software/igv/ExpressionData)
    ...
    for cell_line in [1, 2]:
        ...
        merged_train_list = merged_train_info.loc[:, [
            "gene_name", "chr", "gene_start", "gene_end", "gex"]].values.tolist()
...
if __name__ == '__main__':
    ...
    for key in GCT_frame_dict.keys():
        GCT_frame_dict[key].to_csv(os.path.join(
            data_small_folder, f"X{key}_trainAndVal.gct"), header=True, index=None, sep="\t")
