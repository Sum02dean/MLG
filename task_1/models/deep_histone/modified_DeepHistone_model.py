...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, grow_rate, conv_ksize, conv_padsize):
        ...
        self.block = nn.Sequential(
            ...
            nn.ReLU(),
            # here 9 and 4 are choosed to keep raw dimention intact
            nn.Conv2d(in_planes, grow_rate, (1, conv_ksize),
                      1, (0, conv_padsize)),
            # nn.Dropout2d(0.2)
        )
...
class DenseBlock(nn.Module):
    def __init__(self, nb_layers, in_planes, grow_rate, conv_ksize, conv_padsize):
        ...
        for i in range(nb_layers):
            layers.append(BasicBlock(in_planes + i*grow_rate,
                          grow_rate, conv_ksize, conv_padsize,))
...
class ModuleDense(nn.Module):
    def __init__(self, SeqOrDnase='seq', bins=None, inside_ksize=[9, 4]):
        ...
        if self.SeqOrDnase == 'seq':
            self.conv1 = nn.Sequential(
                nn.Conv2d(1, 128, (4, self.conv_ksize),
                          1, (0, self.conv_padsize)),
                # nn.Dropout2d(0.2),
            )
        elif self.SeqOrDnase == 'dnase':
            self.conv1 = nn.Sequential(
                # here 7 is number of histone marks
                nn.Conv2d(1, 128, (7, self.conv_ksize),
                          1, (0, self.conv_padsize)),
                # nn.Dropout2d(0.2),
            )
        self.block1 = DenseBlock(
            3, 128, 128, self.conv_ksize, self.conv_padsize)
        ...
        self.block2 = DenseBlock(
            3, 256, 256, self.conv_ksize, self.conv_padsize)
        self.trans2 = nn.Sequential(
            nn.BatchNorm2d(256+3*256),
            nn.ReLU(),
            nn.Conv2d(256+3*256, 512, (1, 1), 1),
            # nn.Dropout2d(0.2),
            # default stride id 4 as kernel size
            nn.MaxPool2d((1, self.tran_ksize)),
        )
    ...
    def forward(self, seq):
        ...
        if self.SeqOrDnase == 'seq':
            ...
            # seq = seq.view(n,1,1,w)  here w=20 .bin size
            seq = seq.view(n, 1, 7, w)
...
class NetDeepHistone(nn.Module):
    ...
    def __init__(self, use_seq, bin_list, inside_ksize):
        ...
        self.dns_map = ModuleDense(
            SeqOrDnase='dnase', bins=self.histone_bins, inside_ksize=inside_ksize)
        self.dns_len = self.dns_map.out_size

        if self.use_seq:
            self.seq_map = ModuleDense(
                SeqOrDnase='seq', bins=self.seq_bins, inside_ksize=inside_ksize)
        ...
        self.linear_map = nn.Sequential(
            ...
            # nn.Linear(925,7), here we only want to predict 1 output/gene expression
            nn.Linear(925, 1),
            nn.ReLU(),
    ...
    def forward(self, seq, dns):
        ...
        # so here actully is strang , why seq not needed flatting ?
        flat_dns = dns.view(dns_n, -1)
...
class DeepHistone():
    ...
    def __init__(self, use_gpu, learning_rate=0.001, use_seq=True, bin_list=[2000, 20], inside_ksize=[9, 4]):
        ...
        self.forward_fn = NetDeepHistone(
            use_seq=use_seq, bin_list=bin_list, inside_ksize=inside_ksize)  # here get general model
        # nn.BCELoss() # change loss function suit for regression model
        self.criterion = nn.MSELoss()
        self.optimizer = optim.Adam(
            self.forward_fn.parameters(), lr=learning_rate, weight_decay=0)
        self.use_gpu = use_gpu
        if self.use_gpu:
            self.criterion, self.forward_fn = self.criterion.cuda(), self.forward_fn.cuda()
    ...
    def train_on_batch(self, seq_batch, dns_batch, lab_batch,):
        ...
        if self.use_gpu:
            seq_batch, dns_batch, lab_batch = seq_batch.cuda(), dns_batch.cuda(), lab_batch.cuda()
    ...
    def test_on_batch(self, seq_batch, dns_batch):
        ...
        dns_batch = Variable(torch.Tensor(dns_batch))
        if self.use_gpu:
            seq_batch, dns_batch,  = seq_batch.cuda(), dns_batch.cuda()
        output = self.forward_fn(seq_batch, dns_batch)
